...
import "@thirdweb-dev/contracts/interfaces/IPack.sol";
...
import { TokenStore, ERC1155Receiver } from "./TokenStore.sol";

contract SpaceWarsPack is
    Initializable,
    ContractMetadata,
    Ownable,
    Royalty,
    ...
    IPack
{
    ...
    address private nativeTokenWrapperAddress;
    ...
    function initialize(        
        ...
        nativeTokenWrapperAddress = _nativeTokenWrapperAddress;
    }

    receive() external payable {
        require(msg.sender == nativeTokenWrapperAddress, "!nativeTokenWrapper.");
    }

    function _nativeTokenWrapper()  internal view virtual override returns (address) {
    ...
    function generateRandomValue() internal view returns (uint256 random) {
        random = uint256(keccak256(abi.encodePacked(_msgSender(), blockhash(block.number - 1), block.prevrandao)));
    }
